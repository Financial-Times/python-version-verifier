...
def test_error_message_returned_on_exception(py_version, required_major, required_minor):
...
def test_required_parameters_returned_on_exception(py_version, required_major, required_minor):
    with pytest.raises(Exception) as exception_resp:
        require_python_3_6(py_version)
    expected_error_msg = 'Detected Python {0}.{1}. '\
        'Python {2}.{3} or above is required, '\
...
def test_required_parameters_return_successful(py_version, required_major, required_minor):
    require_python_3_6(py_version, required_major, required_minor)
